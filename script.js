...
function cerrarSesion() {
    if(confirm("¿Cerrar sesión?")) {
        ...
        const nom = document.getElementById('nombre-login');
        const ape = document.getElementById('apellido-login');
        if(nom) nom.value = "";
        if(ape) ape.value = "";
        location.reload(); 
    }
...
function guardarNuevoVencimiento() {
    ...
    if (tipo === "VTV") {
        ...
        if (!descripcion.trim()) return alert("Escribí la tarea.");
...
function seleccionarUnidad(num, tipo, btn) {
    ...
    // EXCEPCIÓN: Si es Mara Castillo, NO la redirigimos al historial
